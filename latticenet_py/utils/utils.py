...
class Conv2dUB(th.nn.Conv2d):
    def __init__(
        self, in_channels, out_channels, height, width, *args, bias=True, **kwargs
    ):
        """Conv2d with untied bias."""
        super().__init__(in_channels, out_channels, *args, bias=False, **kwargs)
        self.bias = (
...
class GatedConv2dWNSwish(torch.nn.Module):
    def __init__(self, in_channels, out_channels, *args, bias=True, **kwargs):
        ...
        self.gated_conv = Conv2dWN(
            in_channels, out_channels, *args, bias=bias, **kwargs
        )
...
def interpolate_wrapper(cls):
    ...
    class Wrap(cls):
        def __init__(
            self, *args, size=None, scale_factor=None, mode="bilinear", **kwargs
        ):
            check_args_shadowing(
                "interpolate_wrapper",
                super().__init__,
...
def leaky_relu_init(m, alpha=0.2):
    ...
    from latticenet_py.lattice.lattice_modules import (
        CoarsenLatticeModule,
        ConvLatticeIm2RowModule,
        FinefyLatticeModule,
    )
