...
from semanticposs_loader import SemPOSS
...
def run():
    ...
    datadir = '/workspace/data/SemanticPOSS'
    print(f'!!! Looking for data in hardcoded dir {datadir} !!!')
    my_loader_train = DataLoader(SemPOSS(datadir, 'train'), batch_size=1, shuffle=True)
    my_loader_test = DataLoader(SemPOSS(datadir, 'val'), batch_size=1, shuffle=False)

    phases= [
        Phase('train', my_loader_train, grad=True),
        Phase('test', my_loader_test, grad=False)
    ]
    #model
    model=LNN(23, model_params).to("cuda")
    #create loss function
    #loss_fn=GeneralizedSoftDiceLoss(ignore_index=loader_train.label_mngr().get_idx_unlabeled() )
    loss_fn=LovaszSoftmax(ignore_index=0)
    ...
    secondary_fn=torch.nn.NLLLoss(ignore_index=0)  #combination of nll and dice  https://arxiv.org/pdf/1809.10486.pdf
    scheduler=None
    ...
    while True:
        ...
        for phase in phases:
            ...
            pbar = tqdm(total=len(phase.loader))
            # while ( phase.samples_processed_this_epoch < phase.loader.nr_samples()):
            for positions, target in phase.loader:
                positions, target = positions[0,...].to('cuda'), target[0,...].to('cuda')
                print(positions.size(), target.size())
                ...
                with torch.set_grad_enabled(is_training):
                    ...
                    print('make empty values tensor')
                    values = torch.zeros(positions.shape[0], 1).to("cuda")
                    ...
                    print('start after forward pass callback')
                    cb.after_forward_pass(pred_softmax=pred_logsoftmax, target=target, cloud=None, loss=loss.item(), loss_dice=loss_dice.item(), phase=phase, lr=optimizer.param_groups[0]["lr"]) #visualizes the prediction
                    pbar.update(1)
