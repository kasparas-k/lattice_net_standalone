...
class SplatLattice(Function):
    ...
    def forward(ctx, lattice, positions, values):
        ...
        lattice.begin_splat()
        splatting_indices, splatting_weights = lattice.splat_standalone(
            positions, values
        )

        return (
            lattice.values(),
            LatticeWrapper.wrap(lattice),
            splatting_indices,
            splatting_weights,
        )  # Pytorch functions requires all the return values to be torch.Variables so we wrap the lattice into one
...
class DistributeLattice(Function):
    ...
    def forward(ctx, lattice, positions, values, reset_hashmap=True):
        ...
        distributed_lattice, distributed, splatting_indices, splatting_weights = (
            lattice.distribute(positions, values, reset_hashmap)
        )
        ...
        return (
            LatticeWrapper.wrap(distributed_lattice),
            distributed,
            splatting_indices,
            splatting_weights,
        )
    ...
    def backward(ctx, grad_distributed, grad_indices, grad_weights):
        ...
        grad_distributed_values = grad_distributed_values.view(
            nr_positions, pos_dim + 1, val_dim
        )
        grad_distributed_values = grad_distributed_values.permute(
            0, 2, 1
        )  # makes it  nr_positions, val_dim, pos_dim+1
        grad_values = grad_distributed_values.sum(
            dim=2
        )  # we sum over the pos_dim+1 vertices that we distributed over
...
class ExpandLattice(Function):
    ...
    def forward(
        ...
        expanded_lattice = lattice_structure.expand(
            positions, point_multiplier, noise_stddev, expand_values
        )

        ctx.nr_values_original_lattice = lattice_structure.nr_lattice_vertices()

        return expanded_lattice.values(), LatticeWrapper.wrap(
            expanded_lattice
        )  # Pytorch functions requires all the return values to be torch.Variables so we wrap the lattice into one
...
class Im2RowIndicesLattice(Function):
    ...
    def forward(ctx, lattice_values, lattice, filter_extent, dilation, nr_filters):
        ...
        lattice_rowified = lattice.im2rowindices(
            lattice, filter_extent, dilation, False
        )
    ...
    def backward(ctx, grad_lattice_rowified):
        ...
        grad_values = lattice.row2im(
            grad_lattice_rowified, dilation, filter_extent, nr_filters, lattice
        )
...
class Im2RowLattice(Function):
    ...
    def backward(ctx, grad_lattice_rowified):
        ...
        if lattice.val_dim() != val_dim:
            print(
                "something went wrong. Lattice val dim is ",
                lattice.val_dim(),
                " but val dim is ",
                val_dim,
            )
...
class ConvIm2RowLattice(Function):
    ...
    def forward(ctx, lattice_values, lattice, filter_bank, dilation):
        ...
        convolved_lattice = lattice.convolve_im2row_standalone(
            filter_bank, dilation, lattice, False
        )
        ...
        return convolved_lattice.values(), LatticeWrapper.wrap(
            convolved_lattice
        )  # Pytorch functions requires all the return values to be torch.Variables so we wrap the lattice into one
    ...
    def backward(ctx, grad_lattice_values, grad_lattice_structure):
        ...
        filter_bank_backwards = filter_bank.transpose(
            0, 1
        )  # creates a nr_filters x filter_extent * val_fim
        filter_bank_backwards = filter_bank_backwards.view(
            nr_filters, filter_extent, val_dim
        )  # nr_filters x filter_extent x val_fim
        filter_bank_backwards = filter_bank_backwards.transpose(
            0, 1
        ).contiguous()  # makes it filter_extent x nr_filters x val_fim   #TODO the contigous may noy be needed because the reshape does may do a contigous if needed or may also just return a view, both work
        filter_bank_backwards = filter_bank_backwards.reshape(
            filter_extent * nr_filters, val_dim
        )
        lattice.set_values(grad_lattice_values)
        grad_lattice = lattice.convolve_im2row_standalone(
            filter_bank_backwards, dilation, lattice, True
        )
...
class CoarsenLattice(Function):
    ...
    def forward(
        ...
        filter_bank,
        coarsened_lattice=None,
    ):  # Can offer a precomputed coarsened lattice
        ...
        if coarsened_lattice == None:
            coarsened_lattice = lattice_fine_structure.create_coarse_verts_naive(
                positions
            )

        # convolve at this lattice vertices with the neighbours from lattice_fine
        dilation = 1
        convolved_lattice = coarsened_lattice.convolve_im2row_standalone(
            filter_bank, dilation, lattice_fine_structure, False
        )
    ...
    def backward(ctx, grad_lattice_values, grad_lattice_structure):
        ...
        # reconstruct lattice_rowified
        lattice_fine_structure.set_values(lattice_fine_values)
        lattice_rowified = coarsened_lattice.im2row(
            lattice_fine_structure, filter_extent, dilation, False
        )
        ...
        grad_lattice_py = lattice_fine_structure.convolve_im2row_standalone(
            filter_bank_backwards, dilation, coarsened_lattice, True
        )
...
class FinefyLattice(Function):
    ...
    def forward(
        ...
        dilation = 1
        convolved_lattice = lattice_fine_structure.convolve_im2row_standalone(
            filter_bank, dilation, lattice_coarse_structure, False
        )
    ...
    def backward(ctx, grad_lattice_values, grad_lattice_structure):
        ...
        # reconstruct lattice_rowified
        lattice_coarse_structure.set_values(lattice_coarse_values)
        lattice_rowified = lattice_fine_structure.im2row(
            lattice_coarse_structure, filter_extent, dilation, False
        )
        ...
        # one hast o convolve at the fine positions, having the neighbour as the coarse ones because they are the ones with the errors
        grad_lattice_py = lattice_coarse_structure.convolve_im2row_standalone(
            filter_bank_backwards, dilation, lattice_fine_structure, True
        )
        grad_lattice = grad_lattice_py.values()
...
class SliceLattice(Function):
    ...
    def forward(
        ...
        if splatting_indices == None and splatting_weights == None:
            sliced_values, splatting_indices, splatting_weights = (
                lattice_structure.slice_standalone_no_precomputation(positions)
            )
        else:
            sliced_values = lattice_structure.slice_standalone_with_precomputation(
                positions, splatting_indices, splatting_weights
            )

        ctx.save_for_backward(
            positions, sliced_values, splatting_indices, splatting_weights
        )
    ...
    def backward(ctx, grad_sliced_values):
        ...
        positions, sliced_values_hom, splatting_indices, splatting_weights = (
            ctx.saved_tensors
        )
        ...
        if lattice_structure.val_dim() is not grad_sliced_values.shape[1]:
            sys.exit(
                "for some reason the values stored in the lattice are not the same dimension as the gradient. What?"
            )
        # lattice_py.set_val_dim(grad_sliced_values.shape[1])
        grad_sliced_values = grad_sliced_values.contiguous()
        lattice_structure.slice_backwards_standalone_with_precomputation_no_homogeneous(
            positions, grad_sliced_values, splatting_indices, splatting_weights
        )
        lattice_values = (
            lattice_structure.values()
        )  # we get a pointer to the values so they don't dissapear when we realease the lettice
...
class SliceClassifyLattice(Function):
    ...
    def forward(
        ...
        class_logits = lattice_structure.slice_classify_with_precomputation(
            positions,
            delta_weights,
            linear_clasify_weight,
            linear_clasify_bias,
    ...
    def backward(ctx, grad_class_logits):
        ...
        lattice_py.slice_classify_backwards_with_precomputation(
            grad_class_logits,
            positions,
            initial_values,
            delta_weights,
...
class GatherLattice(Function):
    ...
    def forward(
        ...
        gathered_values = lattice_structure.gather_standalone_with_precomputation(
            positions, splatting_indices, splatting_weights
        )
    ...
    def backward(ctx, grad_sliced_values):
        ...
        # lattice_py.set_val_dim(val_dim)
        lattice_py.gather_backwards_standalone_with_precomputation(
            positions, grad_sliced_values, splatting_indices, splatting_weights
        )
        lattice_values = (
            lattice_py.values()
